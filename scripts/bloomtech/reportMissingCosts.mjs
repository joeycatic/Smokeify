...
const run = async () => {
  ...
  const bloomtechProducts = await prisma.product.findMany({
    where,
    orderBy: { title: "asc" },
    select: {
      id: true,
  ...
  const rows = [];
  for (const product of bloomtechProducts) {
  ...
  console.log(
    `[report] bloomtechProducts=${bloomtechProducts.length} missingCostVariants=${rows.length} shown=${limitedRows.length}`
  );

  if (!limitedRows.length) {
    console.log("[report] No Bloomtech variants with missing cost.");
    return;
  }
