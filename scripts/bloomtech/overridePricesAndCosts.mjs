...
const run = async () => {
  const {
    ...
    parseArgs();
  const allowWrite = process.env.BLOOMTECH_REPRICE_ALLOW_WRITE === "1";
  ...
  if (!apply || !allowWrite) {
    console.log(
      "[override] Dry run only. Set BLOOMTECH_REPRICE_ALLOW_WRITE=1 and pass --apply to write."
    );
  }
  ...
  const where = buildBloomtechFilter({
  ...
  const results = {
    bloomtechProductsFound: products.length,
    inspected: 0,
    updatedProducts: 0,
    updatedVariants: 0,
