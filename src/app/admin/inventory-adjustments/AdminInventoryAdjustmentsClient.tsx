...
export default function AdminInventoryAdjustmentsClient({
  ...
  return (
    <section className="rounded-xl border border-[var(--adm-border)] bg-[var(--adm-surface)] p-5 shadow-[0_20px_56px_rgba(2,6,23,0.32)]">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-[0.28em] text-[var(--adm-text-faint)]">
            Manual adjustments
          </p>
          <h2 className="mt-2 text-xl font-semibold text-[var(--adm-text)]">Create audited stock corrections</h2>
          <p className="mt-2 max-w-2xl text-sm text-[var(--adm-text-muted)]">
        ...
        {selectedVariant ? (
          <div className="rounded-xl border border-[var(--adm-primary)] bg-[var(--adm-primary-soft)] px-4 py-3 text-xs text-[var(--adm-primary)]">
      ...
      {error ? (
        <div className="mt-4 rounded-xl border border-rose-500/20 bg-[#fae7e3] px-4 py-3 text-sm text-[var(--adm-error)]">
          {error}
        </div>
      ) : null}
      {notice ? (
        <div className="mt-4 rounded-xl border border-emerald-500/20 bg-[var(--adm-primary-soft)] px-4 py-3 text-sm text-[var(--adm-success)]">
          {notice}
        </div>
      ) : null}
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[0.95fr_1.05fr]">
        <div>
          ...
          />
          {searching ? <p className="mt-3 text-sm text-[var(--adm-text-faint)]">Searching…</p> : null}
          <div className="mt-3 space-y-2">
            {results.map((variant) => (
              ...
              >
                ...
                <span className="text-xs text-[var(--adm-primary)]">On hand {variant.quantityOnHand}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
            <label className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[var(--adm-text-faint)]">
              ...
              <select
                ...
                className="mt-2 h-9 w-full rounded-xl border border-[var(--adm-border)] bg-[var(--adm-surface)] px-4 text-sm text-[var(--adm-text)] outline-none"
              >
          ...
          <label className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[var(--adm-text-faint)]">
            ...
            <textarea
              ...
              className="mt-2 w-full rounded-xl border border-[var(--adm-border)] bg-[var(--adm-surface)] px-4 py-3 text-sm text-[var(--adm-text)] outline-none placeholder:text-[var(--adm-text-faint)]"
            />
          </label>
          ...
          <button
            ...
            className="inline-flex h-9 items-center justify-center rounded-xl bg-[var(--adm-primary)] px-5 text-sm font-semibold text-white transition hover:bg-[var(--adm-primary-dim)] disabled:cursor-not-allowed disabled:bg-slate-700 disabled:text-[var(--adm-text-muted)]"
          >
