...
export default async function AdminCompliancePage() {
  ...
  return (
    <div className="admin-legacy-page space-y-6">
      ...
      <div className="space-y-3 md:hidden">
        {products.map((product) => {
          ...
          return (
            <div
              key={product.id}
              className="rounded-[24px] border border-white/10 bg-[#090d12] px-4 py-4 text-sm text-slate-300 shadow-[0_18px_50px_rgba(0,0,0,0.22)]"
            >
              <Link
              ...
              <div className="mt-1 break-all text-xs text-slate-500">{product.handle}</div>
              <div className="mt-4 grid grid-cols-2 gap-3">
                <ComplianceMeta label="Storefronts" value={product.storefronts.join(", ")} />
                <ComplianceMeta
                  label="Compliance"
                  value={`${product.complianceStatus} · Feed ${
                    product.complianceFeedEligible ? "on" : "off"
                  } · Ads ${product.complianceAdsEligible ? "on" : "off"}`}
                />
              </div>
              <div className="mt-4">
      ...
      <div className="hidden overflow-hidden rounded-[28px] border border-white/10 bg-[#090d12] shadow-[0_18px_50px_rgba(0,0,0,0.22)] md:block">
...
function ComplianceMeta({ label, value }: { label: string; value: string }) {
  return (
